...
- `Presenter` - Взаимодействие между слоями данных и представления реализовано в файле `index.ts`, выполняющем роль презентера. Оно осуществляется через события, обрабатываемые брокером событий и описанными в `index.ts` обработчиками.
...
- `items:changed` Событие генерируется при изменении списка доступных товаров.
- `item:selected` Событие возникает при выборе пользователем конкретного товара из списка. 
- `preview:changed` Событие вызывается при изменении товара, отображаемого в режиме предварительного просмотра. 
- `basket:changed` Событие генерируется при любом изменении содержимого корзины (добавление или удаление товаров).
- `basket:delete` Событие вызывается, когда пользователь удаляет товар из корзины.
- `item:moved` Событие генерируется при добавлении товара в корзину.
- `modal:open` Событие вызывается при открытии модального окна. Блокировка прокрутки основной страницы.
- `modal:close` Событие вызывается при закрытии модального окна. Разблокировка прокрутки основной страницы.
- `order:opened` Событие генерируется при переходе пользователя к оформлению заказа.
- `input:error` Событие вызывается при обнаружении ошибок в форме ввода данных. Обновление ошибок в формах (заказа или контактов). Проверка валидности полей формы.
- `input:change` Событие вызывается при изменении значения поля ввода. Сохранение введенных данных в состояние приложения.
- `order:submit` Событие генерируется при отправке формы заказа.
- `contacts:submit` Событие вызывается при отправке формы контактных данных.
- `order:success` Событие вызывается при успешной обработке заказа на сервере.
...
Методы:
- `set phone(value: string)` сеттер устанавливает значение поля телефона.
- `set email(value: string)` сеттер устанавливает значение поля email.
- `clear()` очищает поля ввода email и тел.
...
Методы: 
- `set total` сеттер устанавливает значение общей суммы товаров.
