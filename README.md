...
Методы:
`isValid()` Проверяет, заполнены ли все обязательные поля формы. Используется для активации кнопки отправк
...
Поля:
`totalAmount: HTMLElement` разметка общей суммы товаров в заказе
